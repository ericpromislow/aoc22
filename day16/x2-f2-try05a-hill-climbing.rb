...
Choose teams for two initial partitions

Then do hill-climbing

For each member of team1
- For each member of team2
  - swap & recalculate
  - keep the best

Does this get us to 2400?
...
class NodesList
  attr_reader :nodes, :costs, :distances, :dpUses
  def initialize
    @nodes = {}
    @distances = {}
  end
  ...
  def stateKey(node, timeLeft, children)
    return "T:#{timeLeft}, N:#{node.name}, C:#{children.sort.join(":")}"
  end  
...
#    puts "numChecks needed: #{@numChecks}"
  ...
  def evaluateAux(node, timeLeft, childrenLeft)
    ...
    dpKey = nil
    if validChildren.size > 0 && validChildren.size < 6
      dpKey = stateKey(node, timeLeft, validChildren)
      if $dp.has_key?(dpKey)
        $dpUses[validChildren.size] += 1
        return $dp[dpKey]
      end
    end
=begin
    if validChildren.size >= 5
      possibleGains = validChildren.map{|childName| [nodes[childName].gain * (timeLeft - cost[childName]), childName ]}
      maxGain = possibleGains.max {|a, b| a[0] <=> b[0] }[0]
      threshold = maxGain / 4
      newChildren = possibleGains.filter { |x| x[0] >= threshold }.map{|x| x[1]}
      if newChildren.size < validChildren.size
        # puts "node: #{node.name}, timeLeft:#{timeLeft}, reduce #{validChildren.size} to #{newChildren.size}"
...
=end
    ...
    if dpKey
      $dp[dpKey] = thisGain + maxChildGain
    end
...
#    pp nodeList2.costs
    ...
    puts "iter: #{iter}: maxPartitionedScore:#{maxPartitionedScore}, maxScore1: #{maxScore1}, maxScore2: #{maxScore2} => #{maxScore1 + maxScore2}, dpusage: #{$dpUses}"
...
puts "dpusage: #{nodeList1.dpUses}, #{nodeList2.dpUses}"
